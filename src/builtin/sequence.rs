...
use crate::{manage_native, to_native, to_primitive, unpack_native, unpack_types, CompilationError, RTCell, RootCompilationScope, XCallableSpec, XStaticFunction, XType, xraise, forward_err, xraise_opt};
...
impl<W: Write + 'static> XSequence<W> {
    ...
    pub(super) fn get(
        ...
        match self {
            ...
            Self::Map(seq, func) => {
                ...
                ns.eval_func_with_values(f, vec![original], rt, false).map(|e| Ok(e.unwrap_value()))?
            }
            Self::Zip(sequences) => {
                let items = sequences
                    .iter()
    ...
    pub(super) fn slice<'a>(
        ...
        ns: &'a RuntimeScope<W>,
        rt: RTCell<W>,
    ) -> impl DoubleEndedIterator<Item=Result<EvaluatedVariable<W>, RuntimeError>> + 'a {
    ...
    fn sorted(
        ...
        ns: &RuntimeScope<W>,
        rt: RTCell<W>,
    ) -> Result<Result<Option<Self>, String>, RuntimeError> {
        let arr = self.slice(ns, rt.clone()).collect::<Result<Vec<_>, _>>()?;
        // first we check if the seq is already sorted
        let mut is_sorted = true;
        for w in arr.windows(2) {
            if to_primitive!(
                match ns.eval_func_with_values(cmp_func, vec![w[0].clone(), w[1].clone()], rt.clone(), false)?.unwrap_value(){
                    Ok(i) => i,
        ...
        if is_sorted {
            ...
            let mut ret = arr;
            forward_err!(try_sort(&mut ret, |a, b| -> Result<Result<_, String>, RuntimeError> {
                Ok(Ok(
                    to_primitive!(forward_err!(ns.eval_func_with_values(cmp_func, vec![a.clone(), b.clone()], rt.clone(), false)?.unwrap_value()),
                    Int
                )
                        .is_negative()))
            })?);
    ...
    scope.add_func(
        ...
        XStaticFunction::from_native(
            |args, ns, _tca, rt| {
                ...
                let idx = xraise!(value_to_idx(arr, idx));
    ...
    scope.add_func(
        ...
        XStaticFunction::from_native(
            |args, ns, _tca, rt| {
                ...
                let idx = to_primitive!(a1, Int);
                let idx = xraise!(value_to_idx(seq, idx));
                let mut ret = seq.slice(ns, rt.clone()).take(idx).collect::<Result<Vec<_>, _>>()?;
                ret.push(a2);
                ret.extend(
                    // todo remove collect
                    seq.slice(ns, rt.clone()).skip(idx).collect::<Result<Vec<_>, _>>()?
                );
        ...
        XStaticFunction::from_native(
            |args, ns, _tca, rt| {
                ...
                ret.extend(
                    ...
                    seq.slice(ns, rt.clone()).skip(idx + 1).collect::<Result<Vec<_>, _>>()?
                );
                Ok(manage_native!(XSequence::array(ret), rt))
            },
        ),
    ...
    scope.add_func(
        ...
        XStaticFunction::from_native(
            |args, ns, _tca, rt| {
                ...
                let mut idx1 = xraise!(value_to_idx(seq, idx1));
                let mut idx2 = xraise!(value_to_idx(seq, idx2));
                if idx1 == idx2 {
                    return Ok(a0.clone().into());
                }
                if idx1 > idx2 {
                ...
                ret.extend(
                    // todo avoid collecting
                    seq.slice(ns, rt.clone()).take(idx2).skip(idx1 + 1).collect::<Result<Vec<_>,_>>()?
                );
                ret.push(seq.get(idx1, ns, rt.clone())?);
                ret.extend(
                    // todo avoid collecting
                    seq.slice(ns, rt.clone()).skip(idx2 + 1).collect::<Result<Vec<_>,_>>()?
                );
                Ok(manage_native!(XSequence::array(ret), rt))
            },
        ),
    ...
    scope.add_func(
        ...
        XStaticFunction::from_native(
            |args, ns, _tca, rt| {
                ...
                let seq = to_native!(a0, XSequence<W>);
                if seq.is_empty() {
                    return xraise!(Err("sequence is empty".to_string()));
                }
                let f = to_primitive!(a1, Function);
    ...
    scope.add_func(
        ...
        XStaticFunction::from_native(
            |args, ns, _tca, rt| {
                ...
                if args.len() == 1 {
                    let a0 = xraise!(eval(&args[0], ns, &rt)?);
                    end = xraise!(to_primitive!(a0, Int).to_i64().ok_or("end out of bounds"));
                    start = 0i64;
                    step = 1i64;
                } else {
                    let a0 = xraise!(eval(&args[0], ns, &rt)?);
                    ...
                    start = xraise!(to_primitive!(a0, Int)
                        .to_i64()
                        .ok_or("start out of bounds"));
                    end = xraise!(to_primitive!(a1, Int).to_i64().ok_or("end out of bounds"));
                    step = xraise!(a2.map_or(Ok(1i64), |a2| {
                        to_primitive!(a2, Int).to_i64().ok_or("step out of bounds")
                    }));
                }
                if step.is_zero() {
                    xraise!(Err("invalid range, step size cannot be zero".to_string()))
        ...
        XStaticFunction::from_native(
            |args, ns, _tca, rt| {
                ...
                for (i, item) in items.by_ref().enumerate() {
                    let item = item?;
                    let res = xraise!(ns.eval_func_with_values(f, vec![item.clone()], rt.clone(), false)?.unwrap_value());
        ...
        XStaticFunction::from_native(
            |args, ns, _tca, rt| {
                ...
                if matches_left.is_zero() {
                    return xraise!(Err("match_count must be non-zero".to_string()));
                }
                ...
                for item in arr {
                    let item = item?;
                    if *to_primitive!(xraise!(ns.eval_func_with_values(f, vec![item.clone()], rt.clone(), false)?.unwrap_value()), Bool) {
        ...
        Ok(XFunctionFactoryOutput::from_native(
            ...
            move |args, ns, _tca, rt| {
                ...
                for (x, y) in arr0.into_iter().zip(arr1.into_iter()) {
                    let eq = xraise!(ns.eval_func_with_values(inner_eq_func, vec![x?, y?], rt.clone(), false)?.unwrap_value());
