...
pub fn add_float_type(scope: &mut XCompilationScope, interner: &mut StringInterner) -> Result<(), CompilationError> {
    scope.add_native_type(interner.get_or_intern_static("float"), X_FLOAT.clone())
}

macro_rules! add_float_binop {
    ($fn_name:ident, $name:ident, $func:expr) => {add_binop!($fn_name, $name, X_FLOAT, Float, X_FLOAT, $func);};
}
...
add_binop!(add_float_eq, eq, X_FLOAT, Float, X_BOOL, |a: &f64,b: &f64| {
...
add_ufunc!(add_float_floor, floor, X_FLOAT, Float, X_INT, |a:&f64| Ok(XValue::Int(whole_float_to_bigint(a.floor())).into()));
add_ufunc!(add_float_ceil, ceil, X_FLOAT, Float, X_INT, |a:&f64| Ok(XValue::Int(whole_float_to_bigint(a.ceil())).into()));
add_ufunc!(add_float_trunc, trunc, X_FLOAT, Float, X_INT, |a:&f64| Ok(XValue::Int(whole_float_to_bigint(a.trunc())).into()));

add_ufunc!(add_float_to_str, to_str, X_FLOAT, Float, X_FLOAT, |a:&f64| {
    Ok(XValue::String(a.to_f64().ok_or("rational cannot be converted to float")?.to_string()).into())
});

add_binop!(add_float_cmp, cmp, X_FLOAT, Float, X_INT, |a,b| Ok(xcmp(a,b)));