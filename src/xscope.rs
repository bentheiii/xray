...
use crate::{CompilationResult, CompilationError, TracedCompilationError, XExplicitArgSpec, XStaticExpr, XExplicitFuncSpec, UfData, XCompoundFieldSpec, XCallableSpec, Bind};
...
impl<'p> XCompilationScope<'p> {
                        ...
                        let ancestor_overloads = ancestor_overloads.iter().map(|x| {
                            if let XFunctionFactory::Static(stat) = x.as_ref() {
                                if let XStaticFunction::Recourse(spec, ..) = stat.as_ref() {
                                    Rc::new(XFunctionFactory::Static(Rc::new(XStaticFunction::Recourse(spec.clone(), depth))))
                                } else {
                                    x.clone()
                                }
                            } else {
    ...
    pub fn feed(&mut self, input: Pair<Rule>, parent_gen_param_names: &HashSet<String>, interner: &mut StringInterner, runtime: RTCell) -> Result<Vec<Declaration>, TracedCompilationError> {
        match input.as_rule() {
...
impl<'p> XEvaluationScope<'p> {
    ...
    pub fn get_ud_func(&self, name: DefaultSymbol)->Option<Rc<XStaticFunction>>{
        self.ud_functions.get(&name).cloned().or_else(|| self.parent.as_ref().and_then(|parent| parent.get_ud_func(name)))
    }
